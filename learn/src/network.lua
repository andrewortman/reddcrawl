...
local function gru(inputSize, rnnSize, gateSquash, dropoutRate)
    ...
    local input_sum = function() 
        local gate = nn.CAddTable()({
            nn.Linear(inputSize, rnnSize)(input):annotate{name="input_transform", graphAttributes=styleLinear},
    ...
    local candidate = nn.Tanh()(nn.CAddTable()({
        nn.Linear(inputSize, rnnSize)(input):annotate{graphAttributes=styleLinear}, --it is the sum of the input
...
function network.create(rnnSize, rnnLayers, dropoutRate, gateSquash)
    ...
    for i = 1, rnnLayers do
        ...
        local gruLayer = gru(previousLayerSize, rnnSize, gateSquash, dropoutRate)({previousLayer, prevH}):annotate{name="gru["..i.."]", graphAttributes=styleGRU}
        table.insert(outputTable, gruLayer) -- do not dropout through time!

        local dropoutLayer = nn.Dropout(dropoutRate)(gruLayer):annotate{name="gru-dropout["..i.."]", graphAttributes=styleDropout}
        previousLayer = dropoutLayer
        previousLayerSize = rnnSize
    end

    -- output layers linear transforms
    local outputLinear = nn.Linear(previousLayerSize, outputSize)(previousLayer):annotate{name="out_l1", graphAttributes=styleLinear}
